...
export const GlobalPulseInfo = ({
  ...
  return (
    <Card className="p-4 border-primary/20 cyber-glow h-full my-0 px-[28px] py-[23px] flex flex-col">
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="text-center p-3 rounded-lg bg-muted/30">
          <div className={`text-2xl font-bold mb-1 ${intensityInfo.color}`}>
            {globalIntensity}%
          </div>
          <div className="text-xs text-muted-foreground">Intensidade</div>
        </div>
        <div className="text-center p-3 rounded-lg bg-muted/30">
          <div className="text-2xl font-bold text-primary mb-1">{Math.floor(Math.random() * 50) + 120}</div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="flex flex-col items-center p-2 rounded-lg bg-destructive/10">
          <div className="w-3 h-3 rounded-full bg-destructive mb-2"></div>
      ...
      <div className="p-4 rounded-lg bg-gradient-to-r from-muted/50 to-muted/20 mb-6">
        <div className="flex items-center justify-between mb-3">
          <span className="text-sm font-medium">Nível Regional</span>
          <Badge variant="outline" className={`text-sm ${intensityInfo.color} border-current`}>
            {intensityInfo.level}
          </Badge>
        </div>
      ...
      <div className="flex-1 min-h-0">
        ...
        <div className="space-y-3 overflow-y-auto max-h-32">
          {recentAlerts.map(alert => (
            <div key={alert.id} className="flex items-start gap-3 p-2 rounded-lg bg-muted/20 hover:bg-muted/40 transition-colors">
